...
## 2. Methoden
Hier bespreken we de Behavior Parameters die het gedrag van de agent bepalen, en het Agent-script dat de logica van observaties, acties en beloningen stuurt.
...
Daarnaast is een Ray Perception Sensor 3D component toegevoegd die er voor zorgt dat de agent kan kijken. Deze sensor zendt raycasts uit die objecten met de tags “Menhir” en “Destination” detecteren. De straallengte is zo ingesteld dat Obelix objecten over het bijna volledige speelveld kan waarnemen.
...
- OnEpisodeBegin()
Wordt aangeroepen bij het begin van elke trainingsepisode. Deze methode reset de volledige staat: de snelheid en positie van Obelix worden gereset, alle eerder gespawnde objecten worden verwijdered, en nieuwe objecten worden ge-initiate. Zes destinations worden in een cirkelvormige opstelling geplaatstmet een instelbare radius, terwijl zes menhirs op willekeurige posities geplaatst worden op het speelveld.
- CollectObservations
...
Maakt het mogelijk om de agent handmatig te besturen via het toetsenbord (pijltjestoetsen of WASD) voor te testen. Met AD kan je draaien, WS kan je voor / achteruit bewegen.
